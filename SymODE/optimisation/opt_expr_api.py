...
def _de_optimize(cfg: OptimizeConfig):
    ...
    keys = ["C1","C2","C3"]
    # init population
    pop: List[Tuple[float,float,float,float,float,float]] = []
    for _ in range(cfg.de_pop):
        C1 = rng.uniform(*cfg.bounds_phys["C1"])
        C2 = rng.uniform(*cfg.bounds_phys["C2"])
        C3 = rng.uniform(*cfg.bounds_phys["C3"])
    ...
    for g in range(1, cfg.de_gens+1):
        for i in range(cfg.de_pop):
            ...
            v = [A[0] + cfg.de_F*(B[0]-C[0]),
                 A[1] + cfg.de_F*(B[1]-C[1]),
                 A[2] + cfg.de_F*(B[2]-C[2])]
            # binomial crossover + clipping
            jrand = rng.integers(3)
            trial = [0.0,0.0,0.0]
            for j,k in enumerate(keys):
                lo,hi = cfg.bounds_phys[k]
                if rng.random() < cfg.de_CR or j == jrand:
                    trial[j] = float(np.clip(v[j], lo, hi))
                else:
                    trial[j] = pop[i][j]
            Q6, Q6pen = _eval_Q6_and_pen(trial[0], trial[1], trial[2], cfg)
            fit = Q6 if cfg.objective=="raw" else Q6pen
            if fit >= pop[i][5]:
                pop[i] = [trial[0], trial[1], trial[2], Q6, Q6pen, fit]
                if fit >= best[5]:
                    best = pop[i].copy()
        hist.append((g, best[3], best[4]))

    pop.sort(key=lambda x: x[5], reverse=True)
    topk = pop[:cfg.topk]
    best_dict = {"C1":best[0], "C2":best[1], "C3":best[2], "Q6":best[3], "Q6_pen":best[4], "fitness":best[5]}
