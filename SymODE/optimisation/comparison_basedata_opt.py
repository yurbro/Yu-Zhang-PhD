...
for _, row in df_train.iterrows():
    ...
    ax1.plot(time_hours, mean_vals, color="tab:blue", alpha=0.6, linewidth=1)
...
for _, row in df_test.iterrows():
    mean_vals = row[mean_cols].to_numpy(dtype=float)
    std_vals  = row[std_cols].to_numpy(dtype=float)
    ax1.plot(time_hours, mean_vals, color="tab:orange", alpha=0.6, linewidth=1)
    ax1.fill_between(time_hours,
                     mean_vals - std_vals,
                     mean_vals + std_vals,
...
ax1.set_ylabel("Cumulative amount of ibu release (µg/cm²)")
...
# —— 图 1 保存 ——
fig1.savefig(
    os.path.join(out_dir, "IVRT_release_profiles_train_test_multiopt.png"),
    dpi=300,
    bbox_inches="tight"
)
...
for _, row in df_all.iterrows():
    mean_vals = row[mean_cols].to_numpy(dtype=float)
    std_vals  = row[std_cols].to_numpy(dtype=float)
    ax2.plot(time_hours, mean_vals, color="tab:blue", alpha=0.5, linewidth=1)
...
hist_line = Line2D([0], [0], color="tab:blue", lw=2, label="History data")
ax2.legend(handles=[hist_line] + opt_handles2, loc="best")

ax2.set_xlabel("Time (h)")
ax2.set_ylabel("Cumulative amount of ibu release (µg/cm²)")
...
# —— 图 2 保存 ——
fig2.savefig(
    os.path.join(out_dir, "IVRT_release_profiles_historical_data_multiopt.png"),
    dpi=300,
    bbox_inches="tight"
)
