...
def save_top_candidates(dim, pareto_front, ei_vals, df, path_data, run_num, method, total, k):
    ...
    if method == f'EI-{dim}D':
        # just save EI, select top total candidates by EI
        if np.all(ei_vals <= 0):
        ...
        # just save HV, select top total candidates by HV contribution
        ...
        # Default Hybrid: top k candidates by EI + top (total-k) candidates by HV
        k_EI = k
        k_HV = total - k
...
def run_acquisition_function(dim, run_num, path_af, path_data, path_data_run, method, total, k, benchmark, savefig):
    ...
    # based on the method, save top candidates
