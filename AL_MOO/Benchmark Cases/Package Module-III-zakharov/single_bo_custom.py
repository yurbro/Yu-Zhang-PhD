...
def bayesian_optimization(objective_func, X_init, Y_init, bounds, n_iter, af_func, benchmark, acquisition_func):
    ...
    if record:
        ...
        save_path = fr"Multi-Objective Optimisation\Benchmark\Package Module-III-zakharov\BO-RE\single_bo_{af_func}_{benchmark}_{dim}D.xlsx"
        try:
...
def ackley_function(**kwargs) -> float:
    ...
    return float((-(sum1 + sum2**2 + sum2**4)).item())

def zakharov_function(**kwargs) -> float:
    """
    Calculate standard Zakharov function value (minimization objective).
    Input:
...
if __name__ == "__main__":
    ...
    benchmark = 'Zakharov'  # Benchmark function Zakharov
    acquisition_func = {
        'ei': expected_improvement,
        'ucb': upper_confidence_bound,
        'poi': probability_of_improvement
    }
    lb, ub = np.array([-10] * dim), np.array([10] * dim)  # Lower and upper bounds
    ...
    dim = X_init.shape[1]
    bounds = {f'x{i+1}': (-10, 10) for i in range(dim)}
