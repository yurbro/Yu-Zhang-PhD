...
def lhs_samples(n: int, lb: np.ndarray, ub: np.ndarray) -> np.ndarray:
    """ Perform Latin Hypercube Sampling (LHS) over each dimension range [lb[i], ub[i]] and return shape (n, d) """
    rng = np.random.default_rng(seed=42)  # Set random seed for reproducibility
    d = len(lb)
    result = np.zeros((n, d))
    for j in range(d):
        # Generate n random points within each partition
...
if __name__ == "__main__":
    n_init = 20         # Initial number of samples
