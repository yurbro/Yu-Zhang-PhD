...
def save_top_candidates(dim, pareto_front, ei_vals, df, path_data, run_num, method, total, k):
    """
    save_top_candidates(dim, pareto_front, ei_vals, df, path_data, run_num, method, total, k)
    Save the top candidates based on the acquisition function values.
    Parameters:
    -----------
    dim : int
        Dimension of the problem
    pareto_front : pd.DataFrame
        DataFrame containing the Pareto front points
    ei_vals : np.ndarray
        Array containing the EI values for the candidates
    """
    save_path = f'{path_data}\\Top-RUN{run_num}-{method}.xlsx'

    if method == f'EI-{dim}D':
        # just use EI, save EI最大的total个点
        if np.all(ei_vals <= 0):
        ...
        # just use HV, save HV contribution最大的total个点
        ...
        # default to Hybrid: top k_EI candidates by EI + top k_HV candidates by HV
        k_EI = k
        k_HV = total - k
...
def run_acquisition_function(dim, run_num, path_af, path_data, path_data_run, method, total, k, benchmark, savefig):
    ...
    # force conversion to numeric, drop NaN values
    df["Mean"] = pd.to_numeric(df["Mean"], errors="coerce")
    df["Std"] = pd.to_numeric(df["Std"], errors="coerce")
    df = df.dropna(subset=["Mean", "Std"])

    df["Num"] = df.index + 1  # add a sequence column
