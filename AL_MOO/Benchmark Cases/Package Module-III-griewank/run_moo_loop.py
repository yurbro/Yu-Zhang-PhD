...
def run_moo_loop(path, path_data, path_df, d, lb, ub,
    ...
    # concatenate RUN-{run_num-1}-{method} 和 RUN-{run_num}-{method} 's contents，and write back to RUN-{run_num}-{method} sheet
    ...
    with pd.ExcelFile(file_path_optimisation) as xls:
        ...
        if sheet_prev in xls.sheet_names and sheet_curr in xls.sheet_names:
            ...
            # base on the dimension, select the columns to keep
...
if __name__ == "__main__":
    ...
    lb = np.array([-600] * d)  # Lower bounds
    ub = np.array([600] * d)   # Upper bounds
    method = f'EI-{d}D'  # Method name for the optimisation; can be 'EI', 'HV', 'RANDOM', or 'PROPOSED' 
    benchmark = 'Ackley'  # Benchmark function to be used, which is Zakharov in this case; more benchmarks can be added later
    directory = "Multi-Objective Optimisation\\Benchmark\\Package Module-III-griewank\\Dataset"  # Directory to save the initial experiment data
    directory_af = "Multi-Objective Optimisation\\Benchmark\\Package Module-III-griewank\\Results" # Directory to save the acquisition function results
    lower_bound, upper_bound = 1e-2, 1e2  # Define the bounds for tuning the GPR model
