...
if __name__ == "__main__":
    ...
    df_X = pd.read_excel(file_path, sheet_name='Formulas')
    df_Y = pd.read_excel(file_path, sheet_name='C')  # 每列是一个配方在10个时间点的Q(t)
    ...
    for run in range(iter):
        ...
        }).to_csv(f"Symbolic Regression/srloop/data/variable_index_mapping_run-{run + 1}.csv", index=False)
