#!/usr/bin/python
# -*- encoding: utf-8 -*-
# File    :   sr_Qt2fct_fe_loop_plain.py
...
from sub_StruFilter import extract_substructures, build_new_feature, statistics_for_structure_frequency, replace_var_indices
import sympy
...
def plot_performance(y_raw, y_pred, run):
    ...
    plt.savefig(fr'Symbolic Regression/srloop/other models/Plain SR/visualisation/sr_Qt2fct_fe_prediction_run-{run}.png')
    # plt.show()
...
if __name__ == "__main__":
    ...
    for run in range(iter):
        ...
        }).to_csv(f"Symbolic Regression/srloop/other models/Plain SR/data/variable_index_mapping_run-{run + 1}.csv", index=False)
        ...
        hall_of_fame.to_csv(f"Symbolic Regression/srloop/data/hall_of_fame_run-{run + 1}.csv", index=False)
