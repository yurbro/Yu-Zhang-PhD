...
def create_train_test_sets(file_name):
    ...
    def create_train_test_sets(df):
        while True:
            ...
            train_set = df_shuffled.iloc[:21]
            test_set = df_shuffled.iloc[21:]
            ...
            if len(below_max) >= 2 and len(above_max) >= 2:
                below_max = below_max.sample(2)
                above_max = above_max.sample(2)
    ...
    # Output the results
    print("Training Set (21 formulas):")
    print(train_set)
    print("\nTesting Set (4 formulas):")
