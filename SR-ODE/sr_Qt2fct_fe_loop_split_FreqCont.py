...
"""
    ...
    Update: added the frequency and complexity (6) guided sub-structure filtering strategy.
            added the maximum complexity penalty update.
"""
import pandas as pd
import numpy as np
...
from subStruFilterCom import build_new_feature, statistics_for_structure_frequency
...
if __name__ == "__main__":
    ...
    iter = 10 # 迭代次数
    ir = 1 # count of runs
    maxsize = 25 # SR complexity max size
    maxsize_penalty = 25 # SR complexity max size
    ...
    for run in range(iter):
        ...
        model = PySRRegressor(
            ...
            loss="loss(x, y) = (x - y)^2",
            maxsize=maxsize,  # set up a dynamic size limit when the maximum complexity of feature matrix is reached, the maxsize will be adjusted accordingly. (penalty)
            verbosity=1,
        )
        ...
        # Restore the raw expression
        print("\nRestoring the raw expression for the next run...")
        restored_output_csv = f'Symbolic Regression/srloop/data/hall_of_fame_run-{run + 1}_restored.csv'
        fame_df_restored = restore_equations(fame_csv, mapping_csv, restored_output_csv)
        print(f"✅ Restored output saved to {restored_output_csv}")

        # --- Step Loop-6: Extract substructures & build new feature bank ---
        # update the contribution of sub-structures
        top_structures = statistics_for_structure_frequency(hall_of_fame)
        print("\nExtracting substructures from equations...")
        new_feature_bank, n_febank, max_unique_comp = build_new_feature(top_structures, run + 1, n_febank)
        feature_names_update = feature_names  # 更新特征名称
        feature_matrix_updated = feature_matrix

        maxsize_penalty_updated = maxsize_penalty - max_unique_comp  # 计算最大复杂度惩罚项
        maxsize = min(maxsize_penalty_updated, maxsize)  # 更新最大复杂度限制
        print(f"Updated maxsize after penalty for run {run + 2}: {maxsize}")
