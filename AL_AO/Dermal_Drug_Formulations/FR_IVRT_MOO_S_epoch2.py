...
# Standardize using the scaler from the training set
...
# Print elapsed time
print(f'Time elapsed: \033[93m{str(timedelta(seconds=elapsed_time))}\033[0m')  # Use timedelta to format time
...
class MyProblem(ElementwiseProblem):
    ...
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        y_pred, y_std = self.best_gpr.predict([x], return_std=True)     # Predict using the GPR model
        # ic(x, y_pred)
...
# Set the limits for the axes based on the Pareto fronts
...
# Initialize the vertical line and text objects
...
# Extract the mean and std from the Pareto front results
for i in range(len(res.X)):
    x = res.X[i]
