...
path = 'Multi-Objective Optimisation\Benchmark\Package Module-III-rastrigin\Ave_Improve'
# file_path = path + r"\improvement_comparison - bo - Rastrigin.xlsx"     # compare bo
file_path = path + r"\improvement_comparison - Rastrigin.xlsx"     # compare strategies
...
ax.set_ylim(bottom=0)        # If you want to truncate/zoom in on the numbers, change it here
# ax.set_yscale('log')       # Optional: turn on if the gap is too large

# ========= 6. LEGEND =========
ax.legend(fontsize=11, loc='best')

# ========= 7. SHOW/SAVE =========
plt.tick_params(axis='both', labelsize=12)  # Set the size of the tick labels
plt.tight_layout()
# plt.savefig(path + r'\improvement_comparison_bo_Rastrigin.png', dpi=300, bbox_inches='tight')
plt.savefig(path + r'\improvement_comparison_Rastrigin.png', dpi=300, bbox_inches='tight')
plt.show()
