...
def calculate_improvement(file_path_save, benchmark, method, iter):
    ...
    for i in range(1, iter + 1):
        ...
        # Select the maximum value from df_iter
        df_iter_max = df_iter.max().values[0]
        # print(f"Best value for iteration {i}: {best_value}, Max value in df_iter: {df_iter_max}")
        df_imp = df_iter_max - best_value  # calculate improvement

        # # average improvement calculation
        # df_imp = df_iter.values - best_value  # calculate improvement
...
if __name__ == "__main__":
    ...
    path = "Multi-Objective Optimisation\Benchmark\Package Module-III-griewank\Dataset"
    # method = "RANDOM"  # or "PROPOSED", depending on the method
    iter = 30
    dim = 5  # Dimensions
    alpha = 0.5
    benchmark = "Ackley"  # or "Zakharov", depending on the benchmark function
    # file_path_save = f"Multi-Objective Optimisation\Benchmark\Package Module-III-griewank\Ave_Improve\\{method}_improvement_analysis_a-{alpha}.xlsx"
    # file_path_save = f"Multi-Objective Optimisation\Benchmark\Package Module-III-griewank\Ave_Improve\\{method}_improvement_analysis_a-{alpha}-{dim}D.xlsx"
    # file_path_save = f"Multi-Objective Optimisation\Benchmark\Package Module-III-griewank\Ave_Improve\\{method}_improvement_analysis.xlsx"

    # calc BO_3D results

    method = F"BO_UCB_{dim}D"  # Specify the method for which you want to calculate the improvement
    file_path_save = f"Multi-Objective Optimisation\Benchmark\Package Module-III-griewank\Ave_Improve\\{method}_improvement_analysis.xlsx"   # calc BO_3D results
