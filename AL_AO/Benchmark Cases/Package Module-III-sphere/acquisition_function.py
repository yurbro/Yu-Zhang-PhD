...
def save_top_candidates(dim, pareto_front, ei_vals, df, path_data, run_num, method, total, k):
    ...
    if method == f'EI-{dim}D':
        # just use EI, save top total candidates by EI
        if np.all(ei_vals <= 0):
        ...
        # automatically determine the number of X based on input dimension
        ...
        # just use HV, save top total candidates by HV contribution
        ...
        # randomly select total candidates
        # np.random.seed(5)  # 3D
        np.random.seed(9)  # 5D
        total = min(total, len(pareto_front))  # ensure not exceed pareto_front length
        ...
        # Default case: use both EI and HV, allocate points based on weights
        k_EI = k
        k_HV = total - k
