...
def set_plot_style(dim, alpha_columns_5d, iterations_5d, path, df_5d, alpha_cols_5d, special_cols_5d, benchmark):
    ...
    plt.xticks(range(0, len(df_5d['Iteration']) + 1), fontsize=12)
...
if __name__ == "__main__":
    ...
    # Read data file
    path = 'Multi-Objective Optimisation\\Benchmark\\Package Module-III-sphere\\BO-RE'
    # Test PROPOSED-xD
    dim = 3  # Dimensions
    filename = 'single_bo_results_Sphere.xlsx'  # Modify to your filename
    sheetname = f'Alpha-Comparison-{dim}D'  # Modify to your sheet name
    df_5d = pd.read_excel(path + '/' + filename, sheet_name=sheetname)  # Modify to your file path
    ...
    sheetname_2_5d = f'Alpha-Comparison-{dim}D'  # Modify to your sheet name
    df_5d = pd.read_excel(path + '/' + filename, sheet_name=sheetname_2_5d)  # Modify to your file path
    # Define column groups
    alpha_cols_5d = [col for col in df_5d.columns if col.startswith('Proposed')]
    special_cols_5d = ['BO_EI', 'BO_UCB', 'BO_POI']

    # Call function to plot
    set_plot_style(dim, alpha_columns_5d, iterations_5d, path, df_5d, alpha_cols_5d, special_cols_5d, benchmark='Sphere')
